...
BOOL CALLBACK EnumChildProc(HWND hWnd, LPARAM lParam)
{
	...
	SetWindowText(hWnd, TEXT("здесь был я"));
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		if (wParam == VK_CONTROL)
		{
			...
			SetWindowText(h, TEXT("omg"));
			if (!h)
				MessageBox(hWnd, TEXT("Необходимо открыть \"Калькулятор\""), TEXT("Ошибка!!!"), MB_OK | MB_ICONSTOP);
			else
				EnumChildWindows(h/*дескриптор окна верхнего уровня*/, EnumChildProc, (LPARAM)hWnd); // начинаем перечисление дочерних окон "Калькулятора"
		}
		break;
	default:
