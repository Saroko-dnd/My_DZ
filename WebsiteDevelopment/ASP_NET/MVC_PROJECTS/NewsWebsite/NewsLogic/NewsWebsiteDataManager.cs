...
namespace NewsLogic
{
    ...
    {
        private INewsWebsiteRepository CurrentNewsWebsiteRepository;

        public IEnumerable<Comment> GetAllCommentsForNews(News SelectedNews)
        {
            List<Comment> ListOfComments;
            ListOfComments = CurrentNewsWebsiteRepository.AllComments.Where(CurrentComment => CurrentComment.News == SelectedNews).ToList();
        ...
        {
            List<News> NewsList;
            NewsList = CurrentNewsWebsiteRepository.AllNews.ToList();            
            return NewsList;
        }
        ...
        {
            News FoundNews = null;
            FoundNews = CurrentNewsWebsiteRepository.AllNews.Where(CurNews => CurNews.NewsID == NewsIDForSearch).FirstOrDefault();
            FoundNews.Comments = CurrentNewsWebsiteRepository.AllComments.Where(CurComment => CurComment.NewsID == FoundNews.NewsID).ToList();
            foreach (Comment CurrentComment in FoundNews.Comments)
            {
                CurrentComment.Author = CurrentNewsWebsiteRepository.AllUsers.Where(CurrentUser => CurrentUser.UserID == CurrentComment.AuthorID).FirstOrDefault();
                CurrentComment.LikesAndDislikes = CurrentNewsWebsiteRepository.AllLikesAndDislikes.Where(CurrentUserOpinion => CurrentUserOpinion.CommentID == CurrentComment.CommentID).ToList();
            }
        ...
        public IEnumerable<News> GetHotNews()
        {
            ...
            CurrentCollectionOfImportantNews = CurrentNewsWebsiteRepository.AllNews.Where(CurrentNews => CurrentNews.HotNews == true).ToList();           
        ...
        public void AddNewNews(News NewNews)
        {
            ...
            CurrentNewsWebsiteRepository.AddNews(NewNews);
            CurrentNewsWebsiteRepository.SaveAllChangesMadeInsideCollections();
        ...
        public IEnumerable<News> GetNewsByType(Enums.NewsTypes SelectedTypeOfNews)
        {
            ...
            ListOfNewsWithSelectedType = CurrentNewsWebsiteRepository.AllNews.Where(FoundNews => FoundNews.Type == SelectedTypeOfNews).ToList();    
        ...
        public News UpdateNewsProperty(string PropertyName, object PropertyValue, long CurrentNewsID)
        {
            ...
            ChangedNews = CurrentNewsWebsiteRepository.AllNews.Where(CurrentNews => CurrentNews.NewsID == CurrentNewsID).FirstOrDefault();
